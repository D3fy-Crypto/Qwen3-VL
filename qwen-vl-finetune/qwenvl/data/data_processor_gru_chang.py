...
# chang: trajectory placeholder is the alignment token <gru> (id 151669 in
# BASE_GRU_DIR), so the warm-started projector injects at the same position it
# was aligned on. The embedding row is overwritten by the projector output.
DEFAULT_MOTION_TOKEN = "<gru>"
...
def _build_messages(item: Dict[str, Any], base_path: Path) -> Tuple[List[Dict[str, Any]], bool]:
    ...
    if (
        ...
        # Single <gru> for the whole trajectory, prepended before the original
        # navila navigation prompt (one projected vector per sample).
        user_content = [
            {
                "type": "text",
                "text": (
                    f"This is the encoded trajectory so far: {DEFAULT_MOTION_TOKEN}\\n"
                    "You are a robot agent programmed for navigation tasks. "
                    "Below are historical observations."
                ),
            },
        ]
...
class LazySupervisedDataset(Dataset):
    ...
    def _single_trajectory_actions(self, source_item, is_native, is_envdrop_motion) -> List[int]:
        """Full single-trajectory action list for a sample (-> one <gru> vector).

        Priority: explicit per-sample `gru` field (annotations_with_gru.json) ->
        EnvDrop inline `motion` -> native cumulative actions from the action index.
        Falls back to a single STOP (matches the leading-STOP convention).
        """
        raw = source_item.get("gru")
        if isinstance(raw, list) and len(raw) > 0:
            actions = [int(a) for a in raw if isinstance(a, (int, float))]
            if actions:
                return actions
        if is_envdrop_motion:
            actions = [int(a) for a in (source_item.get("motion") or []) if isinstance(a, (int, float))]
            if actions:
                return actions
        if is_native:
            traj, step = split_video_id(str(source_item.get("video_id", "")))
            actions = self._cumulative_actions_until_inclusive(traj, int(step))
            if actions:
                return actions
        return [STOP] * max(1, int(getattr(self.data_args, "gru_min_seq_len", 1)))
    ...
    def _get_item(self, sources) -> Dict[str, torch.Tensor]:
        ...
        if has_gru:
            # Single trajectory per sample -> one (1, T, 7) feature -> one <gru>.
            actions = self._single_trajectory_actions(source_item, is_native, is_envdrop_motion)
            features = actions_to_motion_features(actions)        # (T, 7)
            gru_features = features.unsqueeze(0)                   # (1, T, 7) single slot
            gru_lengths = torch.tensor([int(features.size(0))], dtype=torch.long)
        else:
            # Non-trajectory sample (ScanQA / video / sharegpt*). No <gru> emitted by
            # _build_messages, so injection is skipped. Dummy 1x1x7 so collator stacks.
        ...
        # chang: pin to the <gru> placeholder so data + model agree on the injection id.
        motion_token_text = DEFAULT_MOTION_TOKEN
